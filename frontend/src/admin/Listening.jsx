  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        >
          <div style={{ ...styles.logo, backgroundColor: '#3b82f6' }}>
            <span style={{ fontSize: '20px' }}>🎧</span>
          </div>
          <div>
            <h1 style={styles.headerTitle}>LearnWithMe</h1>
            <span className="text-[10px] font-bold text-blue-600 tracking-widest uppercase">Thư viện đề thi</span>
          </div>
        </div>
      </header>
        ...
        <div>
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full text-sm font-bold">
              {listeningTests.length} Đề thi hiện có
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {listeningTests.map(test => (
              <div
                ...
                style={{
                  ...styles.testCard,
                  boxShadow: hoveredCard === test.id ? '0 15px 30px rgba(59, 130, 246, 0.1)' : '0 4px 6px rgba(0,0,0,0.02)',
                  borderColor: hoveredCard === test.id ? '#3b82f6' : '#f1f5f9',
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-wrap gap-1">
                    ...
                    <span style={{ ...styles.tag, backgroundColor: '#dbeafe', color: '#1d4ed8' }}>#LISTENING</span>
                  </div>
                ...
                <button
                  style={{ ...styles.button, backgroundColor: '#3b82f6', color: 'white', width: '100%' }}
                  className="hover:bg-blue-700 active:scale-95 transition-all shadow-md shadow-blue-100"
                  onClick={() => handleTestClick(test)}
                >
                  CHI TIẾT ĐỀ THI
                </button>

                <div className="flex items-center justify-center gap-1.5 text-[11px] font-bold text-slate-400 mt-4 cursor-pointer hover:text-blue-600 transition-colors">
            ...
            {listeningTests.length === 0 && (
              <div className="col-span-full py-20 text-center">
                ...
                <p className="text-slate-400 font-medium">Chưa có đề Listening nào trong kho dữ liệu.</p>
              </div>
            )}
          </div>
        </div>
