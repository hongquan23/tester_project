      ...
      <main className="flex-1 overflow-y-auto">
        <div className="px-6 py-10 md:px-20">
          ...
          <div className="mb-10">
            ...
            <p className="text-slate-500 mt-2 italic text-sm">Luyện đọc hiểu với đề thi TOEIC chuẩn format. Hệ thống tự động chấm điểm MCQ.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            {readingTests.map(test => (
              ...
              >
                ...
                <h4 className="text-lg font-bold text-slate-800 line-clamp-2 min-h-[3.5rem] leading-tight hover:text-orange-600 transition-colors cursor-pointer">
                  {test.title || test.name || "Untitled Reading Test"}
                </h4>

                <div className="flex items-center justify-between text-slate-400 text-xs font-semibold py-2 border-y border-slate-50">
                  <span className="flex items-center gap-1"><Clock size={14} /> {test.duration || 75} phút</span>
            ...
            {readingTests.length === 0 && (
              <div className="col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-slate-200">
                <p className="text-slate-400 font-medium">Chưa có đề Reading nào được cập nhật.</p>
              </div>
            )}
          </div>
        </div>
